...
TEST_F(image_operations_TestSuite, copy_paste_cactus) {
    ...
    sprintf(pargs, "-c 90,10,50,100 -i %s -o %s -p 90,60", input_file, actual_output_file);
    INFO(pargs);
...
TEST_F(image_operations_TestSuite, copy_paste_stony1_1) {
    ...
    const char *actual_output_file = "./tests/actual_outputs/result.ppm";
    sprintf(pargs, "-c 5,275,100,75 -p 10,20 -i %s -o %s", input_file, actual_output_file);
    INFO(pargs);
    int status = run_using_system(pargs);
    EXPECT_EQ(0, WEXITSTATUS(status));
    check_image_file_contents(expected_output_file, actual_output_file);
...
TEST_F(image_operations_TestSuite, copy_paste_stony1_2) {
    ...
    const char *actual_output_file = "./tests/actual_outputs/result.ppm";
    sprintf(pargs, "-i %s -p 15,20 -c 75,200,300,10 -o %s", input_file, actual_output_file);
    INFO(pargs);
    int status = run_using_system(pargs);
    EXPECT_EQ(0, WEXITSTATUS(status));
    check_image_file_contents(expected_output_file, actual_output_file);
...
TEST_F(image_operations_TestSuite, copy_paste_stony2_2) {
    ...
    sprintf(pargs, "-i %s -p 20,25 -o %s -c 75,200,50,175", input_file, actual_output_file);
    INFO(pargs);
    int status = run_using_system(pargs);
    EXPECT_EQ(0, WEXITSTATUS(status));
    check_image_file_contents(expected_output_file, actual_output_file);
...
TEST_F(image_operations_TestSuite, copy_paste_stony3_1) {
    ...
    const char *actual_output_file = "./tests/actual_outputs/result.ppm";
    sprintf(pargs, "-i %s -c 120,235,100,150 -o %s -p 20,20", input_file, actual_output_file);
    INFO(pargs);
    int status = run_using_system(pargs);
    EXPECT_EQ(0, WEXITSTATUS(status));
    check_image_file_contents(expected_output_file, actual_output_file);
...
TEST_F(image_operations_TestSuite, copy_paste_stony3_2) {
    ...
    const char *actual_output_file = "./tests/actual_outputs/result.sbu";
    sprintf(pargs, "-c 120,235,100,150 -i %s -o %s -p 55,65", input_file, actual_output_file);
    INFO(pargs);
    int status = run_using_system(pargs);
    EXPECT_EQ(0, WEXITSTATUS(status));
    check_image_file_contents(expected_output_file, actual_output_file);
...
TEST_F(image_operations_TestSuite, copy_paste_stony4_1) {
    ...
    const char *actual_output_file = "./tests/actual_outputs/result.ppm";
    sprintf(pargs, "-i %s -o %s -c 50,60,200,50 -p 50,170", input_file, actual_output_file);
    INFO(pargs);
    int status = run_using_system(pargs);
    EXPECT_EQ(0, WEXITSTATUS(status));
    check_image_file_contents(expected_output_file, actual_output_file);
...
TEST_F(image_operations_TestSuite, copy_paste_stony4_2) {
    ...
    const char *actual_output_file = "./tests/actual_outputs/result.ppm";
    sprintf(pargs, "-i %s -c 25,150,185,125 -o %s -p 50,200", input_file, actual_output_file);
    INFO(pargs);
    int status = run_using_system(pargs);
    EXPECT_EQ(0, WEXITSTATUS(status));
    check_image_file_contents(expected_output_file, actual_output_file);
...
TEST_F(image_operations_TestSuite, copy_paste_stony5_1) {
    ...
    const char *actual_output_file = "./tests/actual_outputs/result.ppm";
    sprintf(pargs, "-o %s -c 100,100,150,50 -p 180,10 -i %s", actual_output_file, input_file);
    INFO(pargs);
    int status = run_using_system(pargs);
    EXPECT_EQ(0, WEXITSTATUS(status));
    check_image_file_contents(expected_output_file, actual_output_file);
...
TEST_F(image_operations_TestSuite, copy_paste_stony5_2) {
    ...
    const char *actual_output_file = "./tests/actual_outputs/result.ppm";
    sprintf(pargs, "-o %s -c 100,100,150,50 -i %s -p 180,10", actual_output_file, input_file);
    INFO(pargs);
    int status = run_using_system(pargs);
    EXPECT_EQ(0, WEXITSTATUS(status));
    check_image_file_contents(expected_output_file, actual_output_file);
...
TEST_F(image_operations_TestSuite, copy_paste_stony6_1) {
    ...
    const char *actual_output_file = "./tests/actual_outputs/result.ppm";
    sprintf(pargs, "-i %s -o %s -c 20,25,120,140 -p 100,200", input_file, actual_output_file);
    INFO(pargs);
    int status = run_using_system(pargs);
    EXPECT_EQ(0, WEXITSTATUS(status));
    check_image_file_contents(expected_output_file, actual_output_file);
...
TEST_F(image_operations_TestSuite, copy_paste_stony6_2) {
    ...
    const char *actual_output_file = "./tests/actual_outputs/result.sbu";
    sprintf(pargs, "-p 100,200 -i %s -o %s -c 20,25,120,140", input_file, actual_output_file);
    INFO(pargs);
    int status = run_using_system(pargs);
    EXPECT_EQ(0, WEXITSTATUS(status));
    check_image_file_contents(expected_output_file, actual_output_file);
